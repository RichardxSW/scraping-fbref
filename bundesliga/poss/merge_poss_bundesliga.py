...
folder = Path("laliga")
# === 1. Baca file ===
laliga_df = pd.read_excel("bundesliga/poss/bundesliga_updated_poss_Freiburg.xlsx")
...
for _, row in barca_for_df.iterrows():
    ...
    mask_home = (laliga_df["date"] == match_date) & (laliga_df["HomeTeam"] == "Mainz 05")
    laliga_df.loc[mask_home, "HPoss"] = poss

    # Real Nottingham Forest sebagai AWAY
    mask_away = (laliga_df["date"] == match_date) & (laliga_df["AwayTeam"] == "Mainz 05")
...
for _, row in barca_against_df.iterrows():
    ...
    mask_home = (laliga_df["date"] == match_date) & (laliga_df["AwayTeam"] == "Mainz 05")
    laliga_df.loc[mask_home, "HPoss"] = poss

    # Lawan Real Nottingham Forest AWAY → possession against masuk ke APoss
    mask_away = (laliga_df["date"] == match_date) & (laliga_df["HomeTeam"] == "Mainz 05")
    laliga_df.loc[mask_away, "APoss"] = poss

# === 4. Simpan hasil ===
laliga_df.to_excel("bundesliga/poss/bundesliga_updated_poss_Mainz.xlsx", index=False)
print("Data possession Mainz 05 berhasil dimasukkan ke HPoss/APoss.")
