...
def scrape_against_possession():
    with sync_playwright() as p:
        ...
        # Klik tab "Against Bayern Munich"
        tab = page.locator("a.sr_preset", has_text="Against Bayern Munich")
        if tab.count() == 0:
            tab = page.locator("text=Against Bayern Munich")

        if tab.count() == 0:
            print("Tab 'Against Bayern Munich' tidak ditemukan.")
        ...
        print("Tab 'Against Bayern Munich' diklik")
        time.sleep(0.5)
    ...
    df.to_csv("pass/bundesliga_pass_against.csv", index=False)
    print(df.head())
    print(f"Total {len(df)} baris disimpan ke bundesliga_pass_against.csv")
