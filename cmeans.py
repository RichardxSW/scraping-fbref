...
# ========= PARAMETER =========
FILE_PATH = "merged_liga1.xlsx"   # ganti sesuai file Excel
SKIP_TOP_ROWS = 0
SHEET_NAME = "contoh"            # <<-- ganti sesuai sheet pemain di file kamu
...
# ========= BACA DATA (level pemain; TANPA agregasi) =========
df_raw = pd.read_excel(FILE_PATH, header=0, skiprows=SKIP_TOP_ROWS, sheet_name=SHEET_NAME)

# Kolom pertama diasumsikan nama pemain; kalau tidak, ganti sendiri di sini:
player_col = df_raw.columns[0]

# Simpan kolom teks opsional untuk anotasi (jika ada)
maybe_team_col = None
...
# Pilih fitur numerik saja (auto-detect)
# num_cols = df_raw.select_dtypes(include=[np.number]).columns.tolist()
num_cols = df_raw.columns[1:10].tolist()
if len(num_cols) == 0:
    raise ValueError("Tidak ditemukan kolom numerik pada sheet pemain. Pastikan ada fitur numerik.")

df_players = df_raw[[player_col] + num_cols].copy()

# Drop baris yang fitur numeriknya semua NaN
df_players = df_players.dropna(subset=num_cols, how="all")

players = df_players[player_col].astype(str).reset_index(drop=True)

# Ambil matriks fitur
X = df_players[num_cols].to_numpy(dtype=float)
feature_names = num_cols

print("[INFO] Jumlah pemain:", len(players))
print("[INFO] Jumlah fitur numerik:", len(feature_names))
print("[INFO] Fitur:", feature_names)
...
u = best_overall["u"]
labels = best_overall["labels_hard"]
centroids_work = best_overall["centroids"]
...
print(f"Silhouette (hard labels) = {sil_avg:.4f}" if sil_avg is not None else
      "Silhouette (hard labels) = N/A (degenerate atau tidak tersedia).")
...
print("\n=== Hasil Cluster per Pemain (FCM hard-label) + Membership Max ===")
...
for cid in range(K_optimal):
    ...
    names = [players.iloc[i] for i in idxs]
...
# anotasi pemain (bisa tambah team/pos jika perlu)
for i in range(coords2d.shape[0]):
    label = f" {players.iloc[i]}"
    # contoh jika ingin tambahkan tim: label = f" {players.iloc[i]} ({df_raw.loc[i, maybe_team_col]})"
    plt.text(coords2d[i, 0], coords2d[i, 1], label, fontsize=8, va="center")

plt.title("Fuzzy C-Means (FCM) — level pemain | warna=cluster (hard), opacity=max membership")
