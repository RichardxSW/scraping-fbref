...
URL = "https://fbref.com/en/squads/17859612/2024-2025/matchlogs/c12/possession/Valladolid-Match-Logs-La-Liga"
...
def scrape_against_possession():
    with sync_playwright() as p:
        ...
        # Klik tab "Against Barcelona"
        tab = page.locator("a.sr_preset", has_text="Against Valladolid")
        if tab.count() == 0:
            tab = page.locator("text=Against Valladolid")

        if tab.count() == 0:
            print("Tab 'Against Valladolid' tidak ditemukan.")
        ...
        print("Tab 'Against Valladolid' diklik")
        time.sleep(0.5)
    ...
    df.to_csv("barcelona_possession_against.csv", index=False)
    print(df.head())
    print(f"Total {len(df)} baris disimpan ke barcelona_possession_against.csv")
