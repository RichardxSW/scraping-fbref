...
URL = "https://fbref.com/en/squads/a2d435b3/2024-2025/matchlogs/c9/possession/Leicester-City-Match-Logs-Premier-League"
...
def scrape_against_possession():
    with sync_playwright() as p:
        ...
        # Klik tab "Against Leicester City"
        tab = page.locator("a.sr_preset", has_text="Against Leicester City")
        if tab.count() == 0:
            tab = page.locator("text=Against Leicester City")

        if tab.count() == 0:
            print("Tab 'Against Leicester City' tidak ditemukan.")
        ...
        print("Tab 'Against Leicester City' diklik")
        time.sleep(0.5)
    ...
    df.to_csv("liverpool_possession_against.csv", index=False)
    print(df.head())
    print(f"Total {len(df)} baris disimpan ke liverpool_possession_against.csv")
