...
def scrape_team_possession():
    with sync_playwright() as p:
        ...
        for row in tbody.find_all("tr"):
            ...
            success_passes = success_pass_cell.get_text(strip=True) if success_pass_cell else None
    ...
    df.to_csv("premier/pass/liverpool_pass.csv", index=False)
    print(df.head())
    print(f"Total {len(df)} baris data possession per match disimpan ke liverpool_pass.csv")
